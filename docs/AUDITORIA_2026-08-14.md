...
- **Bucle de reintentos en RabbitMQ — ALTO, ya observado.** La llamada al modelo ocurre *dentro* del consumidor de la cola. RabbitMQ cierra el canal y **reencola** el mensaje si el consumidor tarda más que `consumer_timeout` (30 min por defecto). **No es hipotético: en la primera prueba del endpoint de flujo, el agente COLLECTIONS tardó 1.875 s (31,25 min) y cruzó ese umbral.** Esta vez la corrida terminó bien, pero si el mensaje se reencola, el mismo trabajo se reprocesa en bucle quemando CPU indefinidamente. **Mitigaciones:** subir `consumer_timeout` explícitamente, poner timeout en la llamada al modelo, y añadir una *dead letter queue*. Con GPU el riesgo casi desaparece, pero la protección debe existir igual.
